...
from magnetron_models.models.qwen3_5_moe import Qwen35MoeModel, Config, CONFIGS
...
def _config_for(repo: str, repo_dir: str) -> Config:
    ...
    if layer_types is not None:  # The interval is what we model, so check it reproduces the checkpoint's pattern.
        ...
        if ours != layer_types:
            raise ValueError(
                f'Layer type mismatch, full_attention_interval={cfg.full_attention_interval} yields {ours}, checkpoint says {layer_types}'
            )
    if text.get('mlp_only_layers'):  # No checkpoint uses these, and a dense layer would need a different block.
...
def _write_model_card(
    ...
    *,
    ...
    with open(path, 'w', encoding='utf-8') as f:
        ...
        f.write('## Qwen3.5-MoE configuration\n\n')
        f.write('| Field | Value |\n')
        f.write('|---|---:|\n')
...
def _convert_model(
    ...
    *,
    ...
    mag_model = Qwen35MoeModel(cfg)  # Not cast as a whole: A_log and dt_bias stay float32, like the reference.
    ...
    with Snapshot.write(snap_file) as snap:
        for shard_path in _iter_safetensor_shards(repo_dir):
            ...
            for key in list(remaining.keys()):  # The checkpoint also carries visual.* and mtp.* tensors, which we skip.
                hf_key: str = hf_key_for(key)
                torch_tensor: torch.Tensor | None = hf_state_dict.get(hf_key)
                if torch_tensor is None:
                    continue
                target_shape, target_dtype = remaining[key]
                ...
                if tuple(out_tensor.shape) != target_shape:
                    raise RuntimeError(f'Shape mismatch for {key}: {tuple(out_tensor.shape)} != {target_shape}')
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Convert a Hugging Face Qwen3.5 MoE model to Magnetron file format')
    parser.add_argument(
        '--model',
        type=str,
