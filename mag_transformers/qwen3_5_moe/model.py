...
from mag_transformers.qwen3_5_moe.config import TextConfig
...
class TextRotaryEmbedding(nn.Module):
    ...
    def forward(self, x: Tensor, position_ids: Tensor) -> [Tensor, Tensor]:
...
def _chunk_gated_delta_rule(
    ...
    if use_qk_l2norm_in_kernel:
        l2norm = lambda x, dim, eps: x * ((x**2).sum(dim=dim, keepdim=True)+eps)
        query = l2norm(query, dim=-1, eps=1e-6)
        key = l2norm(key, dim=-1, eps=1e-6)
    query, key, value, beta, g = [
        x.transpose(1, 2).contiguous().cast(dtype.float32) for x in (query, key, value, beta, g)
    ]
    ...
    last_recurrent_state = (
        Tensor.zeros(batch_size, num_heads, k_head_dim, v_head_dim, dtype=value.dtype)
